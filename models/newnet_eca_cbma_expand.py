...
from .cbma import ResBlock_CBAM
...
class MobileBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, ECA, exp_size):
        ...
        if self.ECA:
            self.eca_block = eca_layer(out_channels, exp_size)
...
class NewNet(nn.Module):
    ...
    def __init__(self, arch: object, num_classes=1000) -> object:
        ...
        self.conv3_256a   = self.__make_layer(256, arch[3], kernel_size=3, padding=1, se=False, exp_size=256)
    ...
    def __make_layer(self, channels, num, kernel_size, padding, se, exp_size):
        ...
        for i in range(num):
            ...
            if se:
                layers.append(ResBlock_CBAM(channels, exp_size//4))
...
class NewNetSE(nn.Module):
    def __init__(self, model_mode="LARGE", num_classes=1000, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            self.block_1 = []
            for in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size in layers_1:
                in_channels = _make_divisible(in_channels * multiplier)
                out_channels = _make_divisible(out_channels * multiplier)
                exp_size = _make_divisible(exp_size * multiplier)
                self.block_1.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size))
            ...
            self.block_2 = []
            for in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size in layers_2:
                in_channels = _make_divisible(in_channels * multiplier)
                out_channels = _make_divisible(out_channels * multiplier)
                exp_size = _make_divisible(exp_size * multiplier)
                self.block_2.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size))
            ...
            self.block_3 = []
            for in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size in layers_3:
                in_channels = _make_divisible(in_channels * multiplier)
                out_channels = _make_divisible(out_channels * multiplier)
                exp_size = _make_divisible(exp_size * multiplier)
                self.block_3.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size))
            ...
            self.block_4 = []
            for in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size in layers_4:
                in_channels = _make_divisible(in_channels * multiplier)
                out_channels = _make_divisible(out_channels * multiplier)
                exp_size = _make_divisible(exp_size * multiplier)
                self.block_4.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size))
            self.block_4 = nn.Sequential(*self.block_4)

            self.block_5 = []
            for in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size in layers_5:
                in_channels = _make_divisible(in_channels * multiplier)
                out_channels = _make_divisible(out_channels * multiplier)
                exp_size = _make_divisible(exp_size * multiplier)
                self.block_5.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, eca, exp_size))
