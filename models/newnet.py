...
class NewNet(nn.Module):
    ...
    def __init__(self, arch: object, num_classes=1000) -> object:
        ...
        self.in_channels = 3
        self.conv3_32 = self.__make_layer(32, arch[0], kernel_size=3, padding=1)
        self.conv3_64 = self.__make_layer(64, arch[1], kernel_size=3, padding=1)
        self.conv3_128 = self.__make_layer(128, arch[2], kernel_size=3, padding=1)
        self.conv3_256a = self.__make_layer(256, arch[3], kernel_size=3, padding=1)
        self.conv3_256b = self.__make_layer(256, arch[4], kernel_size=3, padding=1)
        self.conv1x1_512 = self.__make_layer(512, 1, kernel_size=1, padding=0)
        self.conv1x1_1024 = self.__make_layer(1024, 1, kernel_size=1, padding=0)
        self.conv1x1_out = self.__make_layer(num_classes, 1, kernel_size=1, padding=0)
        
    def __make_layer(self, channels, num, kernel_size, padding):
        layers = []
        for i in range(num):
